...
def fit_data(args):
    ...
    if args.model == 'all':
        ...
        result = fitter.fit_model(phi_ext, current, model_num)
        ...
        if args.plot:
            ...
            ax1.plot(phi_ext, result.fit_curve, 'red', linewidth=2, label='Fit')
            ax1.set_xlabel('Φ_ext')
            ax1.set_ylabel('Current')
            ax1.set_title(f'{result.model.name}\nR² = {result.r_squared:.4f}')
            ax1.legend()
...
def generate_synthetic(args):
    ...
    current = generate_synthetic_data(phi_ext, model_type=args.model,
                                    params=params, noise_level=args.noise)
